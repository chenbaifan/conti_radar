...
: n_(nh), is_sim_(is_sim), buffer_(ros::Duration(100)), listener_(buffer_),
    map_frame_("map"), Radar_frame_("esr_1"), base_link_("base_link") {
        ...
        tf2_filter_ = std::unique_ptr< tf2_ros::MessageFilter<delphi::EsrTrackArray> >(new tf2_ros::MessageFilter<delphi::EsrTrackArray>(buffer_, map_frame_ ,20, *n_));
        tf2_filter_->registerCallback(boost::bind(&EgoCompensater::radar_callback,this,_1));
        // get static tramsform from base_link to ESR
        geometry_msgs::TransformStamped esr2baselink;
        ...
        x_offset = esr2baselink.transform.translation.x;
        y_offset = esr2baselink.transform.translation.y;
        //geometry_msgs::PointStamped origin,transformed;
        //tf2::doTransform(origin, transformed, esr2baselink);
...
void EgoCompensater::compensate_ego_motion(const std::shared_ptr<delphi::EsrTrackArray> detections){
...
custom_msgs::objectList EgoCompensater::convert_to_map_frame(
        const std::shared_ptr<delphi::EsrTrackArray> detected_points) {
...
void EgoCompensater::radar_callback(
        const delphi::EsrTrackArray::ConstPtr detections) {
    ts_ = detections->header.stamp;
    std::shared_ptr<delphi::EsrTrackArray> dets = std::make_shared<delphi::EsrTrackArray>();
    ...
    custom_msgs::objectList radar_points = convert_to_map_frame(std::make_shared<delphi::EsrTrackArray>(cluster_->radar_callback(dets)));
    radar_pub_.publish(radar_points);
}

void EgoCompensater::received_radar(const delphi::EsrTrackArray::ConstPtr detections) {
